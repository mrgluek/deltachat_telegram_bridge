...
- **Refactor: Split the 10,452-line bot.py into 16 focused modules**:
  ...
  - Pure move: no intentional behavior change. Verified after every extraction step with the full test suite (126 tests) and a live `bot.py serve` run.
  - Fixed one real bug along the way that predates this refactor: `_add_channel_bridge` was writing `_history_cooldowns[dc_chat_id]` directly instead of through an accessor, which the module split surfaced as a `NameError` once the dict moved — now goes through `caching.mark_history_cooldown()`.
  - Fixed a second bug the refactor itself would otherwise have introduced: `python bot.py serve` runs this file's top-level code twice per process (once as `__main__`, then again as `bot` via a self-reimport needed so every module's `import bot` resolves to the actual running instance). Once `dc_cli` moved into its own module and became a genuinely shared object across both runs, its `on_init`/`on_start`/event-handler registrations were being added twice, so `on_init` fired twice on every boot. Fixed by registering everything once, from an idempotency-guarded `_register_dc_cli_hooks()` called from `run_cli()`, instead of via top-level `@dc_cli.on(...)` decorators.
