...
- **Telegram RichMessage & Inline Media Support**: Native decoding and relay of posts created with Telegram's Rich Text Editor (featuring inline images, headings, blockquotes, code blocks, lists, and tables interleaved directly inside text, as introduced in Telegram MTProto Layer 229+). Downloads inline media directly via Telethon MTProto without relying on Telegram's web viewer.
- **Public Post Web Recovery & Unsupported Media Handling**: Automatic fallback extraction of public channel posts when Telegram MTProto returns unsupported media objects or rich post containers, preventing cryptic `[MessageMediaUnsupported]` placeholders and ensuring posts without displayable content fall back gracefully to Telegram web links instead of generating empty WebXDC packages.
- **Telegram Rich Post, Video & Album WebXDC Packaging**: Automatically packages long-form Telegram channel posts (with tables, spoilers, blockquotes), video posts, and multi-image albums into standalone, interactive WebXDC applications (`.xdc`). Apps feature offline reading and video playback:
  - **Inline Article Rendering**: Rich Telegram articles (created with Telegram's Rich Text Editor) are rendered as true articles inside the WebXDC app: inline images, figures, and captions appear embedded directly in the article body at the correct position, matching the original Telegram reading experience. All `PageBlock` types are fully supported (paragraphs, photos, collages, slideshows, videos, blockquotes, lists, code blocks, and expandable details — always shown fully expanded).
  - **Unified Gallery Lightbox**: All images in a post (both top gallery and inline article images) participate in a single shared gallery lightbox with previous/next navigation, thumbnail strip, caption, image counter, keyboard arrow keys (←/→/Esc), and touch-swipe gestures.
  - **Embedded Video Playback**: Direct HTML5 `<video controls playsinline preload="metadata">` playback inside the WebXDC app for videos up to 20 MB individually and up to 50 MB total cumulative budget across multiple videos. MP4 streams are stored without CPU-heavy re-encoding for instant packaging and smooth playback across mobile and desktop.
  - **Smart Overflow Handling**: Videos exceeding 20 MB, exceeding the 50 MB package budget, or marked as "Media is too big" by Telegram web embeds are rendered as elegant preview cards with duration badges, poster images, and a prominent «View all videos in Telegram ↗» button linking directly to the Telegram post.
  - **Image, UI & Title Formatting**: Bundles optimized WebP images (max 1280px, matching Telegram's native resolution), channel avatars (128x128 PNG icon extracted directly from local Delta Chat channel cache), dark/light theme (`prefers-color-scheme`), interactive spoilers, table wrapping, lightbox viewer, clear app card titles (`Channel Title #PostID`), and an attribution footer (`Post bridged at ... by Delta Chat Telegram Bridge`) linking to Telegram and source repository. Configurable via `/richmode` (`webxdc`, `split`, `both`, or `off`).
  - **Persistent Media Group Deduplication**: Media groups and multi-photo albums are tracked persistently in SQLite (`processed_media_groups`) with mapping of all individual album message IDs into `message_map`, preventing duplicates across restarts, delayed arrivals, and edit events.
- **Direct Telegram Post Link Previews & WebXDC Packaging**: Automatically intercepts direct Telegram post links (`https://t.me/{channel}/{post_id}` or `https://t.me/s/{channel}/{post_id}`) sent in Delta Chat groups and 1:1 direct chats. Posts are processed using the native MTProto / WebXDC pipeline according to `/richmode` (plain text for text-only posts, photo with caption for single-image posts, and interactive WebXDC `.xdc` apps for rich posts, multi-photo albums, and videos). Includes persistent SQLite caching (`telegram_post_cache`, 24h TTL) to ensure instantaneous 0ms responses on repeated links and prevent Telegram MTProto FloodWait rate limits.
